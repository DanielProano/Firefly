# Resources For RTOS & STM32

## STM32F401xC

### Datasheet
https://www.st.com/resource/en/datasheet/stm32f401cc.pdf

### Reference Manual
https://www.st.com/resource/en/reference_manual/dm00096844-stm32f401xb-c-and-stm32f401xd-e-advanced-arm-based-32-bit-mcus-stmicroelectronics.pdf

Via page 42:

Flash starts at address 0x0000 0000 (256 KB)

SRAM starts at 0x2000 0000 (64KB)

### Reading

#### Linker basics + Memory map
https://www.stf12.org/developers/freerots_ec-linker_script.html

#### Linker walk through
https://itachilab.github.io/linker-script/linker-script.html

#### More indepth walk through
https://allthingsembedded.com/post/2020-04-11-mastering-the-gnu-linker-script/

#### Old everything linker bible
https://users.informatik.haw-hamburg.de/~krabat/FH-Labor/gnupro/5_GNUPro_Utilities/c_Using_LD/ldLinker_scripts.html#SECTIONS_command

#### Another linker walk through
https://charleskorn.com/2016/05/08/a-deeper-look-at-the-stm32f4-project-template-linking-it-all-together/

#### Medium article on linker
https://medium.com/@mkklyci/an-introduction-to-linker-files-crafting-your-own-for-embedded-projects-60ad17193229

#### Startup Assembly
https://github.com/elzoughby/STM32H7xx-Startup

#### Boot Selection & Reset Circuit Comprehensive Guide
https://acheronproject.com/reset_article_1/reset_article_1/